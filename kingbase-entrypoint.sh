...
# ==============================================================================
# 新增的函数：用于处理 /docker-entrypoint-initdb.d/ 目录下的初始化脚本
# ==============================================================================
function docker_process_init_files() {
    echo "[`date`]Processing init files in /docker-entrypoint-initdb.d..."
    # 检查目录是否存在且不为空
    if [ -d "/docker-entrypoint-initdb.d" ] && [ "$(ls -A /docker-entrypoint-initdb.d)" ]; then
        echo "[`date`]Found init files, starting temporary server to run them..."
        # 启动一个临时的后台数据库服务，以便 ksql 可以连接
        ${DB_PATH}/bin/sys_ctl -D ${DATA_DIR} -l ${LOG_FILE} start
        ...
        # 更健壮的等待方式：循环检查直到连接成功
        for i in {1..30}; do
            if ${DB_PATH}/bin/ksql -U ${DB_USER} -d kingbase -c "SELECT 1;" > /dev/null 2>&1; then
        ...
        for f in /docker-entrypoint-initdb.d/*; do
            case "$f" in
                *.sh)     echo "[`date`]running $f"; . "$f" ;;
                *.sql)    echo "[`date`]running $f"; ${DB_PATH}/bin/ksql -U ${DB_USER} -d kingbase < "$f" ;;
                *)        echo "[`date`]ignoring $f" ;;
            esac
        done
        
        # 停止临时数据库
        echo "[`date`]Stopping temporary server..."
        ${DB_PATH}/bin/sys_ctl -D ${DATA_DIR} -m fast stop
    else
        echo "[`date`]No init files found in /docker-entrypoint-initdb.d, skipping."
    fi
}
