...
export const ChatMessageComponent: React.FC<ChatMessageProps> = ({
  ...
  const placeholder = message.role === ChatMessageRole.User ? "prompts.user placeholder" : message.role === ChatMessageRole.System ? "prompts.system placeholder" : "prompts.assistant placeholder";
      ...
      <CardContent className="flex flex-row space-x-1 p-0">
        <div className="min-w-[6rem]">
          <Button
            ...
            className="text-xs"
          >
            {t(`prompts.${capitalize(message.role)}`)}
          </Button>
        </div>
