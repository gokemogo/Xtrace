...
export const ModelParameters: React.FC<ModelParamsContext> = ({
  ...
  return (
    <div className="flex flex-col space-y-4">
      <p className="font-semibold">{t("modelParameters.title")}</p>
      {availableProviders.length === 0 ? (
        <>
          <p className="text-sm">{t("modelParameters.noApiKey")}</p>
          <CreateLLMApiKeyDialog />
        </>
      ) : (
        <div className="space-y-4">
          <ModelParamsSelect
            title={t("modelParameters.provider")}
          ...
          />
          <ModelParamsSelect
            title={t("modelParameters.modelName")}
          ...
          />
          <ModelParamsSlider
            title={t("modelParameters.temperature")}
            ...
            step={0.01}
            tooltip={t("modelParameters.tooltips.temperature")}
            updateModelParam={updateModelParamValue}
          />
          <ModelParamsSlider
            title={t("modelParameters.outputTokenLimit")}
            ...
            max={4096}
            step={1}
            tooltip={t("modelParameters.tooltips.outputTokenLimit")}
            updateModelParam={updateModelParamValue}
          />
          <ModelParamsSlider
            title={t("modelParameters.topP")}
            ...
            max={1}
            step={0.01}
            tooltip={t("modelParameters.tooltips.topP")}
            updateModelParam={updateModelParamValue}
          />
    ...
    <div className="space-y-3" title={tooltip}>
      <div className="flex flex-row">
        ...
        <div className="flex flex-row space-x-3">
          ...
          {setModelParamEnabled ? (
            <Switch
              title={`${t("modelParameters.controlSending")} ${title}`}
              disabled={formDisabled}
              checked={enabled}
