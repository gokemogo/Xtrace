      ...
      <Popover>
        <PopoverTrigger asChild>
          ...
          >
            ...
            {date ? (
              format(date, includeTimePicker ? "PPP pp" : "PPP", { locale: locale === "zh" ? zhCN : undefined })
            ) : (
              <span>{t("date.Pick a date")}</span>
            )}
          </Button>
        </PopoverTrigger>
    ...
    >
          ...
          >
            ...
            {internalDateRange?.from ? (
              internalDateRange.to ? (
                <>
                  {format(internalDateRange.from, "LLL dd, yy : HH:mm", { locale: locale === "zh" ? zhCN : undefined })} -{" "}
                  {format(internalDateRange.to, "LLL dd, yy : HH:mm", { locale: locale === "zh" ? zhCN : undefined })}
                </>
              ) : (
                format(internalDateRange.from, "LLL dd, y", { locale: locale === "zh" ? zhCN : undefined })
              )
            ) : (
              <span>{t("date.Pick a date")}</span>
            )}
      ...
      <Select value={selectedOption} onValueChange={onDropDownSelection}>
        ...
        <SelectContent position="popper" defaultValue={60}>
          ...
          >
            {locale === "zh" ? "日期范围" : DEFAULT_DATE_RANGE_SELECTION}
          </SelectItem>
          {dateTimeAggregationOptions.toReversed().map((item) => (
            <SelectItem key={item} value={item}>
              {item.split(" ")[0]} {t(`date.${item.split(" ")[1]}`)}
            </SelectItem>
          ))}
        </SelectContent>
      </Select>
