...
const getUserNavigation = (t: any) => [
  {
    name: t('common.Language') === 'Language' ? 'Language' : '语言',
    onClick: () => { },
...
export default function Layout(props: PropsWithChildren) {
  ...
  const mapNavigation = (route: Route): NavigationItem | null => {
    ...
    return {
      ...route,
      name: t(route.name), // Use translation key, fallback to original name
  ...
  if (session.status === "loading") return <Spinner message={t('common.loading')} />;
  ...
  return (
    <>
      <Head>
        <title>
          {currentPathName ? `${currentPathName} | Deeptrace` : "Deeptrace"}
        </title>
        <link
            ...
            <nav className="flex h-full flex-1 flex-col overflow-y-auto px-6 pb-3">
              <ul role="list" className="flex h-full flex-col">
                ...
                <ProjectNavigation
                  ...
                  projects={projects}
                />
                <MainNavigation nav={topNavigation} className="mt-8" />
                {/* <MainNavigation nav={bottomNavigation} className="mt-auto" /> */}
                {/* <FeedbackButtonWrapper
                  className="space-y-1"
                  title={t('common.provideFeedback')}
                  description={t('common.feedbackDescription')}
                  type="feedback"
                >
        ...
        {nav.map((item) => (
          <li key={item.name}>
            {(!item.children || item.children.length === 0) && item.href ? (
              ...
              >
                {item.icon && (
                  <MyIcon name={item.icon} size={22} color="inherit" className={clsx(
                    item.current
                      ? "text-primary-accent"
              ...
              >
                {({ open }) => (
                  <>
                    ...
                    >
                      {item.icon && (
                        <MyIcon name={item.icon} size={22} className="h-5 w-5 shrink-0 text-muted-foreground group-hover:text-primary-accent" />
                      )}
                      {item.name}
                      {item.label && (
                        <span
