...
export const SessionPage: React.FC<{
  ...
  if (session.error?.data?.code === "UNAUTHORIZED")
    return <ErrorPage message={t("error.You do not have access to this session")} />;
    ...
    <div className="flex flex-col overflow-hidden xl:container">
      ...
      <div className="flex flex-wrap gap-2">
        {session.data?.users.filter(Boolean).map((userId) => (
          ...
          >
            <Badge>
              {t("sessions.userIdPrefix")} {userId}
            </Badge>
          </Link>
        ))}
        <Badge variant="outline">
          {t("sessions.tracesPrefix")} {session.data?.traces.length}
        </Badge>
        {session.data && (
          <Badge variant="outline">
            {t("sessions.totalCostPrefix")} {usdFormatter(session.data.totalCost, 2, 2)}
          </Badge>
        )}
      </div>
