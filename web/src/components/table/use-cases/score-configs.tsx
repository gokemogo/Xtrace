...
function getConfigRange(
  ...
  if (isNumericDataType(dataType)) {
    return {
      [t("score.Minimum")]: range.minValue ?? "-∞",
      [t("score.Maximum")]: range.maxValue ?? "∞",
    };
  }
...
export function ScoreConfigsTable({ projectId }: { projectId: string }) {
  ...
  const columns: LangfuseColumnDef<ScoreConfigTableRow>[] = [
    ...
    },
    {
      ...
      id: "dataType",
      header: t("common.Data Type"),
      enableHiding: true,
      cell: ({ row }) => {
        const { dataType } = row.original;
        return t(`filters.${dataType}`);
      },
    },
    {
      accessorKey: "range",
      id: "range",
      header: t("common.Range"),
      enableHiding: true,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return t(`TableFilter.${isArchived ? "Archived" : "Active"}`);
      },
    },
    {
      accessorKey: "action",
          ...
          <Popover key={configId}>
            ...
            <PopoverContent>
              <h2 className="text-md mb-3 font-semibold">
                {isArchived ? t("settings.Restore config") : t("settings.Archive config")}
              </h2>
              <p className="mb-3 text-sm">
                {t("settings.Your config is currently")}{" "}
                {isArchived
                  ? t("settings.ScoreConfigsAction", { name })
                  : t("settings.ScoreConfigsActionActive", { name })}
              </p>
