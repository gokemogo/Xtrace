    ...
    <>
      {isPrettyViewAvailable ? (
        <div className="flex flex-row justify-between">
          ...
          >
            <TabsList>
              <TabsTrigger value="pretty">{t("io.pretty")}</TabsTrigger>
              <TabsTrigger value="json">{t("io.json")}</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      ) : null}
      {isPrettyViewAvailable && currentView === "pretty" ? (
        <>
          {inChatMlArray.success ? (
            ...
            <>
              {!(hideIfNull && !input) ? (
                <MarkdownOrJsonView title={t("io.input")} content={input} />
              ) : null}
              {!(hideIfNull && !output) ? (
                <MarkdownOrJsonView
                  title={t("io.output")}
            ...
            <Fragment key={index}>
              <div>
                {!!message.content &&
                  (shouldRenderMarkdown ? (
                    <MarkdownOrJsonView
                      title={message.name ?? t(`prompts.${message.role}`)}
                ...
                {!!message.json && (
                  <JSONView
                    title={
                      message.content ? undefined : message.name ?? t(`prompts.${message.role}`)
                    }
                    json={message.json}
                    className={cn(
                      "bg-muted",
              ...
              {isCollapsed !== null && index === 0 ? (
                ...
                >
                  {isCollapsed
                    ? `${t("io.show")} ${messages.length - COLLAPSE_THRESHOLD} ${t("io.moreEllipsis")}`
                    : t("io.hideHistory")}
                </Button>
              ) : null}
            </Fragment>
