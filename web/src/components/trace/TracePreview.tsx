...
export const TracePreview = ({
  ...
  const { t } = useLanguage();
  const canViewScores = useHasAccess({
    projectId: trace.projectId,
    scope: "scores:CUD",
  });

  // if user doesn't have access and the query param is set to scores, redirect to preview
  if (!canViewScores && selectedTab === "scores") {
    ...
    <Card className="col-span-2 flex max-h-full flex-col overflow-hidden">
      ...
      <div className="flex w-full flex-col overflow-y-auto">
        <CardHeader className="flex flex-row flex-wrap justify-between gap-2">
          <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {!!trace.release && (
                <Badge variant="outline">{t("trace.releasePrefix")} {trace.release}</Badge>
              )}
              {!!trace.version && (
                <Badge variant="outline">{t("trace.versionPrefix")} {trace.version}</Badge>
              )}
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          {selectedTab === "scores" && canViewScores && (
            <ScoresTable
              projectId={trace.projectId}
