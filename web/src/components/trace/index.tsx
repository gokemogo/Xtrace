...
export function TracePage({ traceId }: { traceId: string }) {
  ...
  if (trace.error?.data?.code === "UNAUTHORIZED")
    return <ErrorPage message={t("traces.Error Page")} />;
  if (!trace.data) return <div>{t("common.loading")}...</div>;
  return (
    <div className="flex flex-col overflow-hidden 2xl:container md:h-[calc(100vh-2rem)]">
      <Header
        title={t("traces.Trace Detail")}
        breadcrumb={[
          {
            name: t("navigation.traces"),
      ...
      <div className="flex flex-wrap gap-2">
        {trace.data.sessionId ? (
          <Link
            href={`/project/${router.query.projectId as string
              }/sessions/${encodeURIComponent(trace.data.sessionId)}`}
          >
            <Badge>{t("session.Session")}: {trace.data.sessionId}</Badge>
          </Link>
        ) : null}
        {trace.data.userId ? (
          <Link
          ...
          >
            <Badge>{t("session.User ID")}: {trace.data.userId}</Badge>
          </Link>
        ) : null}
        <TraceAggUsageBadge observations={trace.data.observations} />
        {totalCost ? (
          <Badge variant="outline">
            {t("session.Total cost")}: {usdFormatter(totalCost.toNumber())}
          </Badge>
        ) : undefined}
      </div>
      ...
      {selectedTab === "timeline" && (
        <div className="mt-5 flex-1 flex-col space-y-5 overflow-hidden">
          <Alert>
            ...
            <AlertTitle>{t("trace.New Trace Timeline (beta)")}</AlertTitle>
            {/* <AlertDescription>
              {t("trace.We value your feedback")}{" "}
              <a
