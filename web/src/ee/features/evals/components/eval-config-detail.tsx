...
export const EvalConfigDetail = () => {
  ...
  if (config.data && config.data.evalTemplate === null) {
    return <div>{t("evalConfigs.Config not found")}</div>;
  }
  ...
  return (
    <div className="md:container">
      <Header
        title={config.data?.id ?? `${t("common.loading")}...`}
      ...
      {existingEvalConfig && (
        <>
          <div className="my-5 flex items-center gap-4 rounded-md border p-2	">
            <Label>{t("evalConfigs.Eval Template")}</Label>
            <TableLink
          ...
          <Tabs defaultValue="logs">
            <TabsList>
              <TabsTrigger value="logs">{t("evalConfigs.Logs")}</TabsTrigger>
              <TabsTrigger value="configuration">{t("evalConfigs.Configuration")}</TabsTrigger>
            </TabsList>
      ...
      <PopoverContent>
        ...
        <div className="flex justify-end space-x-4">
          <Button
            ...
            onClick={onClick}
          >
            {t("evalConfigs.Deactivate Eval Job")}
          </Button>
        </div>
      </PopoverContent>
    </Popover>
