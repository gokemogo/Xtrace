          ...
          >
            <SelectTrigger className="text-primary ring-transparent focus:ring-0 focus:ring-offset-0">
              <SelectValue
                ...
                placeholder={t("evalTemplates.SelectTemplatePlaceholder")}
              />
            </SelectTrigger>
      ...
      <div className="col-span-1 lg:col-span-3">
        <InnerEvalTemplateForm
          ...
          preFilledFormValues={
            ...
            langfuseTemplate
              ? {
                name: t(`evalLog.${langfuseTemplate}`) ?? "",
...
const formSchema = z.object({
  ...
  variables: z.array(
    z.string().min(1, "变量必须至少包含一个字符"),
  ),
  outputScore: z.string().min(1, "输入评分函数"),
  outputReasoning: z.string().min(1, "输入推理函数"),
});
    ...
    <Form {...form}>
            ...
            <div className="col-span-1 row-span-1 lg:col-span-2">
              <FormField
                ...
                render={({ field }) => (
                  <>
                    <FormItem>
                      <FormLabel>{t("evalTemplates.form.nameLabel")}</FormLabel>
                      <FormControl>
                        <Input
                          {...field}
                          placeholder={t("evalTemplates.form.namePlaceholder")}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
        ...
        <div className="col-span-1 flex flex-col gap-6 lg:col-span-2">
          <FormField
            ...
            render={({ field }) => (
              <>
                <FormItem>
                  <FormLabel>{t("evalTemplates.form.promptLabel")}</FormLabel>
                  <FormControl>
                    <Textarea
                      {...field}
                      placeholder={t("evalTemplates.form.promptPlaceholder")}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("evalTemplates.form.scoreLabel")}</FormLabel>
                <FormControl>
                  <Input {...field} placeholder={t("evalTemplates.form.scorePlaceholder")} />
                </FormControl>
                <FormDescription>
                  {t("evalTemplates.form.scoreDescription")}
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t("evalTemplates.form.reasoningLabel")}</FormLabel>
                <FormControl>
                  <Input
                    placeholder={t("evalTemplates.form.reasoningPlaceholder")}
                    {...field}
                  />
                </FormControl>
                <FormDescription>
                  {t("evalTemplates.form.reasoningDescription")}
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
      ...
      {formError ? (
        <p className="text-red text-center">
          <span className="font-bold">{t("evalTemplates.ErrorPrefix")}</span> {formError}
        </p>
      ) : null}
    </Form>
  );
