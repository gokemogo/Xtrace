  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={"outline"} title={t("playground.Save to prompt")} asChild>
          <Link href={`/project/${projectId}/playground`}>
            <FileInput className="mr-1 h-5 w-5" />
            <span>{t("playground.Save as prompt")}</span>
          </Link>
        </Button>
      </PopoverTrigger>
      <PopoverContent>
        <Button className="mt-2 w-full" onClick={handleNewPrompt}>
          {t("playground.Save as new prompt")}
        </Button>
        <Divider />
        <Command className="min-h-[8rem]">
          <CommandInput placeholder={t("playground.Search chat prompts")} />
          <CommandEmpty>{t("playground.No chat prompt found")}</CommandEmpty>
        ...
        >
          {t("playground.Save as new prompt version")}
        </Button>
      </PopoverContent>
    </Popover>
  );
...
export function Divider() {
  ...
  return (
    <div className="my-6 flex flex-row justify-center align-middle">
      ...
      <p className="mx-2 text-sm text-gray-400">{t("playground.or")}</p>
      <div className="flex flex-1 flex-col">
        <div className="flex-1 border-b-2 border-gray-200" />
        <div className="flex-1" />
      </div>
