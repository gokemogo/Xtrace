...
export const TracesBarListChart = ({
  ...
  const transformedTraces = traces.data
    ? traces.data.map((item) => {
      return {
        name: item.traceName ? (item.traceName as string) : "未命名",
    ...
    >
      <>
        <TotalMetric
          ...
          description={t("dashboard.totalTraces")}
        />
        {adjustedData.length > 0 ? (
          ...
          <NoData noDataText={t("dashboard.noData")}>
            <DocPopup
              description={t("dashboard.tracesDocDescription")}
        ...
        <ExpandListButton
          ...
          expandText={
            transformedTraces.length > maxNumberOfEntries.expanded
              ? t("dashboard.showTop", { n: maxNumberOfEntries.expanded })
              : t("dashboard.showAll")
          }
        />
      </>
