...
export function DatasetItemsTable({
  ...
  const columns: LangfuseColumnDef<RowData>[] = [
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center gap-2">
            ...
            />
            <span>{t(`TableFilter.${status}`)}</span>
          </div>
        );
      },
    },
    ...
    },
    {
      ...
      header: t("datasets.Expected Output"),
      id: "expectedOutput",
      enableHiding: true,
      cell: ({ row }) => {
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="h-8 w-8 p-0">
                <span className="sr-only [position:relative]">{t("common.Open menu")}</span>
                <MoreVertical className="h-4 w-4" />
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end">
              <DropdownMenuLabel>{t("models.actions")}</DropdownMenuLabel>
              <DropdownMenuItem
                onClick={() => {
              ...
              >
                ...
                {status === DatasetStatus.ARCHIVED ? t("common.Unarchive") : t("common.Archive")}
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        );
