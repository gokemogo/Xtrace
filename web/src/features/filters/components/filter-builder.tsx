      ...
      >
        <PopoverTrigger asChild>
          <Button variant="outline">
            ...
            <span className="hidden @6xl:ml-2 @6xl:inline">{t("filters.label")}</span>
      ...
      <table className="table-auto">
        <tbody>
          {filterState.map((filter, i) => {
            ...
            return (
              <tr key={i}>
                <td className="p-1 text-sm">{i === 0 ? t("form.Where") : t("form.And")}</td>
                <td className="flex gap-2 p-1">
                  ...
                  >
                    <SelectTrigger className="min-w-[100px]">
                      <SelectValue placeholder={t("filters.column")} />
                    </SelectTrigger>
                    <SelectContent>
                      {columns.map((option) => (
                        <SelectItem key={option.id} value={option.id}>
                          {t(`TableFilter.${option.name}`)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <td className="p-1">
                  {filter.type === "string" ||
                    ...
                    <Input
                      disabled={disabled}
                      value={filter.value ?? ""}
                      placeholder={t("filters.stringPlaceholder")}
                      onChange={(e) =>
                        handleFilterChange(
