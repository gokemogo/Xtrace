...
export function CreateScoreConfigButton({ projectId }: { projectId: string }) {
  ...
  const createScoreConfig = api.scoreConfigs.create.useMutation({
    ...
    onError: (error) =>
      setFormError(error.message ?? t("settings.error occurred config")),
  });
      ...
      >
        <DialogTrigger asChild>
          <Button variant="secondary" loading={createScoreConfig.isLoading}>
            ...
            {t("settings.Add new score config")}
          </Button>
        </DialogTrigger>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("settings.Add new score config")}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form
              className="space-y-6"
            ...
            >
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("common.Data Type")}</FormLabel>
                    <Select
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder="选择一个数据类型" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
              ...
              {isNumericDataType(form.getValues("dataType")) ? (
                <>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("settings.Minimum (optional)")}</FormLabel>
                        <FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t("settings.Maximum (optional)")}</FormLabel>
                        <FormControl>
                          <Input {...field} type="number" />
                        </FormControl>
                        <FormMessage />
                        ...
                        {fields.length > 0 && (
                          <div className="mb-2 grid grid-cols-[1fr,3fr] items-center gap-2 text-left sm:grid-cols-[1fr,7fr]">
                            <FormLabel className="grid grid-flow-col">
                              {t("table.value")}
                              <DocPopup
                                description={t("score.integer value internally", {
                                  type: isCategoricalDataType(
                                    form.getValues("dataType"),
                                  )
                                    ? t("filters.CATEGORICAL")
                                    : t("filters.BOOLEAN")
                                })}
                                size="xs"
                              ></DocPopup>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <>
                    <FormItem>
                      <FormLabel>{t("settings.Description (optional)")}</FormLabel>
                      <FormControl>
                        <Textarea
                          {...field}
                          placeholder={t("settings.optional description")}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
