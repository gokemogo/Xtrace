...
export function DeleteProjectButton(props: { projectId: string }) {
  ...
  return (
    <div>
      <Header title={t("settings.Danger Zone")} level="h3" />
      <Dialog>
        <DialogTrigger asChild>
          <Button variant="destructive" disabled={!hasAccess}>
            {t("project.Delete Project")}
          </Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold  ">{t("project.Delete Project")}</DialogTitle>
            <DialogDescription className=" ">{t("project.DeleteConfirm", { confirmMessage })}</DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form
