      ...
      <Dialog>
        <DialogTrigger asChild>
          <Button variant="destructive" disabled={!hasAccess}>
            {t("buttons.Transfer Ownership")}
          </Button>
        </DialogTrigger>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle className="text-lg font-semibold">
              {t("settings.Transfer project")}{project?.name ? ` (${project.name})` : ""}
            </DialogTitle>
            <DialogDescription>
              {t("settings.ownership of this project")}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form
            ...
            >
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t("settings.New owner")}</FormLabel>
                    <FormControl>
                      <Input
