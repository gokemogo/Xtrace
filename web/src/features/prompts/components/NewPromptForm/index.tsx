...
export const NewPromptForm: React.FC<NewPromptFormProps> = (props) => {
  ...
  useEffect(() => {
    ...
    if (!isNewPrompt) {
      form.setError("name", { message: t("errors.Prompt name already exist") });
    } else if (currentName === "new") {
      form.setError("name", { message: t("errors.Prompt name cannot be new") });
    } else if (currentName && !/^[\u4e00-\u9fffa-zA-Z0-9_.-]+$/.test(currentName)) {
      form.setError("name", {
        message:
          t("errors.Name must be alphanumeric"),
      });
    } else {
  ...
  return (
    <Form {...form}>
      ...
      >
              ...
              return (
                <div>
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder={t("prompts.Select a prompt name")} {...field} />
                    </FormControl>
                    ...
                    {form.getFieldState("name").error ? (
                      <div className="flex flex-row space-x-1 text-sm font-medium text-destructive">
                        ...
                        {errorMessage?.includes("already exist") ? (
                          ...
                          >
                            {t("prompts.Create a new version for it here")}{" "}
                            <ArrowTopRightIcon />
                          </Link>
                        ) : null}
                      </div>
        ...
        <>
          <FormItem>
            <FormLabel>{t("navigation.prompts")}</FormLabel>
            <Tabs
            ...
            >
              {!initialPrompt ? (
                <TabsList className="flex w-full">
                  ...
                  >
                    {t(`prompts.${capitalize(PromptType.Text)}`)}
                  </TabsTrigger>
                  <TabsTrigger
                    disabled={
                      Boolean(initialPromptContent) &&
                  ...
                  >
                    {t(`prompts.${capitalize(PromptType.Chat)}`)}
                  </TabsTrigger>
                </TabsList>
              ) : null}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>{t("table.config")}</FormLabel>
              <JsonEditor
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center space-x-3 space-y-0 rounded-md border p-4">
              ...
              <div className="space-y-1 leading-none">
                <FormLabel>{t("prompts.Serve prompt as default to SDKs")}</FormLabel>
              </div>
              {currentIsActive ? (
                <div className="text-xs text-muted-foreground">
                  {t("prompts.This makes the prompt available to the SDKs immediately")}
                </div>
              ) : null}
            </FormItem>
          )}
        ...
        >
          {!initialPrompt ? t("prompts.Create prompt") : t("prompts.Update prompt")}
        </Button>
      </form>
      {formError && (
