...
import { PRODUCTION_LABEL, PRODUCTION_LABEL_CN } from "@/src/features/prompts/constants";
...
export function SetPromptVersionLabels({ prompt }: { prompt: Prompt }) {
  ...
  const isPromotingToProduction =
    (!prompt.labels.includes(PRODUCTION_LABEL) ||
      !prompt.labels.includes(PRODUCTION_LABEL_CN)) &&
    (selectedLabels.includes(PRODUCTION_LABEL) ||
      selectedLabels.includes(PRODUCTION_LABEL_CN));
    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          title={t("prompts.Set prompt labels")}
        >
          <FlagIcon className="h-4 w-4" />
        </Button>
      </PopoverTrigger>
      <PopoverContent>
        <h2 className="text-md mb-3 font-semibold">{t("prompts.Prompt version labels")}</h2>
        <h2 className="mb-3 text-xs">
          {t("prompts.Use labels to fetch prompts via SDKs The")} <strong>{t("prompts.production")}</strong>{" "}
          {t("prompts.labeled prompt will be served by default")}
        </h2>
        <Command className="mx-0 my-3 px-0">
          <CommandList className="max-h-full overflow-hidden">
            <CommandSeparator />
            <CommandGroup heading={t("prompts.Promote to production?")}>
              <LabelCommandItem
                {...{
                  selectedLabels,
                  setSelectedLabels,
                  label: t(`prompts.${PRODUCTION_LABEL}`),
                }}
              />
            </CommandGroup>
            <CommandSeparator />
            <CommandGroup heading={t("prompts.Custom labels")}>
              <div
        ...
        >
          {isPromotingToProduction
            ? t("prompts.Save and promote to production")
            : isDemotingFromProduction
              ? t("prompts.Save and remove from production")
              : t("common.save")}
        </Button>
      </PopoverContent>
    </Popover>
