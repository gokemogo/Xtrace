      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-3">
          <Header
            title={prompt.name}
            help={{
              description: t("prompts.prompt Langfuse SDKs"),
            ...
            breadcrumb={[
              ...
              { name: `${t("common.Version")} ${prompt.version}` },
            ]}
            actionButtons={
              <>
                ...
                <Tabs value="editor">
                  <TabsList>
                    <TabsTrigger value="editor">{t("prompts.Editor")}</TabsTrigger>
        ...
        <div className="col-span-2 md:h-full">
          {prompt.type === PromptType.Chat && chatMessages ? (
            <OpenAiMessageView title={t("prompts.Chat prompt")} messages={chatMessages} />
          ) : typeof prompt.prompt === "string" ? (
            <CodeView content={prompt.prompt} title={t("prompts.Text prompt")} />
          ) : (
            <JSONView json={prompt.prompt} title={t("prompts.Prompts")} />
          )}
          <div className="mx-auto mt-5 w-full rounded-lg border text-base leading-7">
            ...
            <div className="flex flex-wrap gap-2 p-3">
              {extractedVariables.length > 0 ? (
                ...
                <span className="text-xs">{t("prompts.No variables")}</span>
              )}
            </div>
          </div>
          ...
          {/* <p className="mt-6 text-xs text-muted-foreground">
            {t("prompts.Fetch prompts via Python or JS/TS SDKs See")}{" "}
            <a
              href="https://langfuse.com/docs/prompts"
              className="underline"
              target="_blank"
            ...
            {t("prompts.for details")}
          </p> */}
          <Accordion type="single" collapsible className="mt-10">
            <AccordionItem value="item-1">
              <AccordionTrigger>
                {t("prompts.Generations using this prompt version")}
              </AccordionTrigger>
              <AccordionContent>
