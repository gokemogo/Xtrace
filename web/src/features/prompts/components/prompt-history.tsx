...
const PromptHistoryTraceNode = (props: {
  ...
  return (
    <div
      className={`group mb-2 flex cursor-pointer flex-col gap-1 rounded-sm p-2 hover:bg-primary-foreground ${props.currentPromptVersion === prompt.version ? "bg-muted" : ""
        }`}
      onClick={() => {
        props.index === 0
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-sm bg-input p-1 text-xs">
          {t("common.Version")} {prompt.version}
        </span>
        {badges}
      </div>
      ...
      <div className="flex gap-2">
        <span className="text-xs text-muted-foreground">
          {t("prompts.by")} {prompt.creator || prompt.createdBy}
        </span>
      </div>
    </div>
  );
