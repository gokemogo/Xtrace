...
export function CreateApiKeyButton(props: { projectId: string }) {
  ...
  return (
    <Dialog open={open} onOpenChange={createApiKey}>
      ...
      >
        <DialogTitle>{t("settings.API keys")}</DialogTitle>
        <div className="shrink overflow-x-hidden overflow-y-scroll">
          <div className="mb-2">
            <div className="text-md font-semibold">{t("common.Secret Key")}</div>
            <div className="my-2">
              {t("settings.generate a new key")}
            </div>
            <CodeView content={generatedKeys?.secretKey ?? "Loading ..."} />
          </div>
          <div>
            <div className="text-md mb-2 font-semibold">{t("common.Public Key")}</div>
            <CodeView content={generatedKeys?.publicKey ?? "Loading ..."} />
          </div>
          <div>
            <div className="text-md mb-2 font-semibold">{t("settings.Host")}</div>
            <CodeView content={hostname} />
          </div>
          {generatedKeys && (
            <div className="mb-2 max-w-full">
              <div className="text-md my-2 font-semibold">{t("settings.Usage")}</div>
              <QuickstartExamples
