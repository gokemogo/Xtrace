...
export function LlmApiKeyList(props: { projectId: string }) {
  ...
  return (
    <div id="llm-api-keys">
      <Header title={t("llmApiKeys.LLM API keys")} level="h3" />
          ...
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-primary md:table-cell">
                {t("llmApiKeys.ProviderName")}
              </TableHead>
              <TableHead className="hidden text-primary md:table-cell">
                {t("llmApiKeys.AdapterLabel")}
              </TableHead>
              <TableHead className="text-primary md:table-cell">
                {t("llmApiKeys.ApiBaseUrlLabel")}
              </TableHead>
              <TableHead className="text-primary">{t("llmApiKeys.SecretKeyLabel")}</TableHead>
              <TableHead />
            </TableRow>
          </TableHeader>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogDescription>{t("llmApiKeys.DeleteConfirmation")}</DialogDescription>
        <DialogFooter>
          <Button
            variant="destructive"
            onClick={() => {
