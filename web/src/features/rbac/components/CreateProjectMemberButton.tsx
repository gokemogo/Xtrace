      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          ...
          >
            ...
            {t("settings.Add new member")}
          </Button>
        </DialogTrigger>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{t("settings.Add new member to project")}</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form
              className="space-y-6"
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder={t("settings.verified email")} />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                    ...
                    <FormDescription>
                      {t("settings.Rights of role")}{": "}
                      {roleAccessRights[field.value].length
                        ? roleAccessRights[field.value].map(item => t(`${field.value}.${item}`))
                        : "none"}
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
