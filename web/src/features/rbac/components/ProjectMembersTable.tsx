...
export function ProjectMembersTable({ projectId }: { projectId: string }) {
  ...
  return (
    <div>
      <Header title={t("project.Project Members")} level="h3" />
      <Card className="mb-4">
        <Table className="text-sm">
          <TableHeader>
            <TableRow>
              <TableHead className="text-primary">{t("table.name")}</TableHead>
              <TableHead className="text-primary">{t("common.Email")}</TableHead>
              <TableHead className="text-primary">{t("common.Role")}</TableHead>
          ...
          <TableBody className="text-muted-foreground">
            {memberships.map((m) => (
              <TableRow key={m.userId} className="hover:bg-transparent">
                ...
                <TableCell>{t(`filters.${m.role}`)}</TableCell>
                {hasDeleteAccess &&
            ...
            <Table className="text-sm">
              <TableHeader>
                <TableRow>
                  <TableHead className="text-primary">{t("common.Email")}</TableHead>
                  <TableHead className="text-primary">{t("common.Role")}</TableHead>
                  <TableHead className="text-primary">{t("project.Sent by")}</TableHead>
                  {hasDeleteAccess ? <TableHead /> : null}
                </TableRow>
              </TableHeader>
              <TableBody className="text-muted-foreground">
