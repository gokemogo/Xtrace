...
const handleSingleEvent = async (
  ...
  const cleanedRaw = cleanEvent(event);
  const parsed = ingestionEvent.safeParse(cleanedRaw);
  if (!parsed.success) {
    // Let zod throw the detailed error as before
    throw new InvalidRequestError(parsed.error.message);
  }

  // Zod object parsing strips unknown keys by default. Some ingestion
  // producers (OTEL processor) add `usageDetails` which is not part of the
  // ingestion schema body, so we restore it from the cleaned raw input so
  // downstream processors (ObservationProcessor) can normalize and persist it.
  const cleanedEvent = parsed.data as any;
  try {
    const raw = cleanedRaw as any;
    if (raw && raw.body && raw.body.usageDetails) {
      cleanedEvent.body = cleanedEvent.body || {};
      cleanedEvent.body.usage = raw.body.usageDetails;
      //cleanedEvent.body.usageDetails = raw.body.usageDetails;
    }
  } catch (e) {
    // ignore restoring failure
  }
