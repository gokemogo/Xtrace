...
import { InternalServerError, getDbType } from "@langfuse/shared";
import {
  ...
  GET: createAuthedAPIRoute({
    ...
    fn: async ({ query, auth }) => {
      ...
      const observationTypeCondition = query.type
        ? dbType === "dm8"
          ? Prisma.sql`AND o."type" = CAST(${query.type} AS VARCHAR2(50))`
      ...
      const fromStartTimeCondition = query.fromStartTime
        ? dbType === "dm8"
          ? Prisma.sql`AND o."start_time" >= CAST(${query.fromStartTime} AS TIMESTAMP WITH TIME ZONE)`
      ...
      const toStartTimeCondition = query.toStartTime
        ? dbType === "dm8"
          ? Prisma.sql`AND o."start_time" < CAST(${query.toStartTime} AS TIMESTAMP WITH TIME ZONE)`
          : Prisma.sql`AND o."start_time" < ${query.toStartTime}::timestamp with time zone at time zone 'UTC'`
        : Prisma.empty;

      const observations = dbType === "dm8"
        ? await prisma.$queryRaw<ObservationView[]>`
            SELECT
              o."id",
              o."name",
