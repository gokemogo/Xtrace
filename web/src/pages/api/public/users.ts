      ...
      const users = dbType === "dm8"
        ? await prisma.$queryRaw`
            WITH model_usage AS (
              SELECT
                user_id,
                TRUNC(o.start_time, 'DD') observation_day,
                o.model,
            ...
            SELECT
              all_users.user_id,
              JSON_ARRAYAGG(JSON_OBJECT(
                  'date' VALUE observation_day,
                  'usage' VALUE daily_usage_json
              )) metrics
            FROM all_users
            LEFT JOIN daily_usage ON all_users.user_id = daily_usage.user_id
            group by 1
            ORDER BY 1
            OFFSET ${(obj.page - 1) * obj.limit} ROWS FETCH NEXT ${obj.limit} ROWS ONLY
          `
        : await prisma.$queryRaw`
            WITH model_usage AS (
              SELECT
