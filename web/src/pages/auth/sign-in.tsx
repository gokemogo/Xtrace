    ...
    Object.entries(authProviders).some(
      ...
      <div>
        {authProviders.credentials && (
          <Divider className="text-muted-foreground">{t("login.or_with", { action })}</Divider>
        )}
...
export default function SignIn({ authProviders, signUpDisabled }: PageProps) {
  ...
  async function onCredentialsSubmit(
    ...
    try {
      ...
      if (result === undefined) {
        setCredentialsFormError(t("login.An unexpected error occurred."));
        ...
        setCredentialsFormError(
          result?.error ?? t("login.An unexpected error occurred."),
        );
      }
    } catch (error) {
  ...
  async function handleSsoSignIn() {
    ...
    if (!email.success) {
      credentialsForm.setError("email", {
        message: t("login.Invalid email address"),
      });
      setSsoLoading(false);
      return;
    }
    ...
    if (!res.ok) {
      setCredentialsFormError(t("login.SSO is not enabled for this domain."));
      setSsoLoading(false);
    } else {
          ...
          <div className="space-y-6">
            ...
            {credentialsFormError ? (
              <div className="text-center text-sm font-medium text-destructive">
                {t(`errors.${credentialsFormError}`)}
                {/* <br />
                {t("errors.Contact support if this error is unexpected")}{" "}
                {env.NEXT_PUBLIC_LANGFUSE_CLOUD_REGION !== undefined &&
                  t("errors.Make sure you are using the correct cloud data region")} */}
              </div>
            ) : null}
