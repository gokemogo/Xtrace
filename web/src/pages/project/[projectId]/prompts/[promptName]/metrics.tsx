...
import { statusBadge } from "@/src/components/layouts/constant";
...
export default function PromptVersionTable() {
  ...
  const columns: LangfuseColumnDef<PromptVersionTableRow>[] = [
          ...
          values && (
            <div className="flex gap-1">
              {values.map((value) => (
                ...
                >
                  {statusBadge.includes(value) ? t(`statusCategories.${value}`) : value}
                </div>
              ))}
            </div>
          )
    ...
    },
    {
      ...
      header: t("table.Median input tokens"),
      enableHiding: true,
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: t("table.Median output tokens"),
      enableHiding: true,
      cell: ({ row }) => {
        const value: number | undefined | null =
    ...
    },
    {
      ...
      id: "generationCount",
      header: t("table.Generations count"),
      enableHiding: true,
      cell: ({ row }) => {
        const value: number | undefined | null =
    ...
    },
    {
      ...
      header: t("table.Average observation scores"),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      header: t("table.Average trace scores"),
      cell: ({ row }) => {
    ...
    },
    {
      ...
      id: "lastUsed",
      header: t("table.Last used"),
      enableHiding: true,
      headerTooltip: {
        description:
          t("table.Last used description"),
    ...
    },
    {
      ...
      id: "firstUsed",
      header: t("table.First used"),
      enableHiding: true,
      headerTooltip: {
        description:
          t("table.First used description"),
        // href: "https://langfuse.com/docs/prompts",
      },
      cell: ({ row }) => {
  ...
  const rows: PromptVersionTableRow[] =
    promptVersions.isSuccess && !!combinedData
      ? combinedData.map((prompt) => ({
        ...
        lastUsed:
          prompt.lastUsed?.toLocaleString() ?? t("common.No linked generation yet"),
        firstUsed:
          prompt.firstUsed?.toLocaleString() ?? t("common.No linked generation yet"),
      }))
      : [];
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] flex-col overflow-hidden xl:container lg:h-[calc(100vh-2rem)]">
      <Header
        ...
        help={{
          description:
            t("prompts.promptName description"),
          // href: "https://langfuse.com/docs/prompts",
        }}
        breadcrumb={[
          {
          ...
          <>
            <Tabs value="metrics">
              <TabsList>
                ...
                <TabsTrigger value="metrics">{t("prompts.Metrics")}</TabsTrigger>
              </TabsList>
            </Tabs>
          </>
        }
