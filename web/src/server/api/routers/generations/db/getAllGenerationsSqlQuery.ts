...
export async function getAllGenerations({
  ...
  const searchCondition = input.searchQuery
    ? dbType === "dm8"
      ? Prisma.sql`AND (
          o."id" LIKE ${`%${input.searchQuery}%`} OR
          o."name" LIKE ${`%${input.searchQuery}%`} OR
          o."model" LIKE ${`%${input.searchQuery}%`} OR
  ...
  const query = dbType === "dm8"
    ? Prisma.sql`
      WITH scores_avg AS (
        SELECT
          trace_id,
          observation_id,
          JSON_OBJECTAGG(name VALUE CAST(avg_value AS DOUBLE PRECISION)) AS scores_avg
        FROM (
          SELECT
            trace_id,
            observation_id,
