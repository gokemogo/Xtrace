...
export class ObservationProcessor implements EventProcessor {
  ...
  async convertToObservation(
    ...
    // Token counts
    // Normalize usageDetails (from OTEL ingestion) into `usage` so the
    // existing token counting and cost logic (which reads `this.event.body.usage`)
    // picks up values like prompt_tokens/completion_tokens/total_tokens.
    if ("usage" in this.event.body && this.event.body.usage) {
      try {
        const ud: Record<string, unknown> = this.event.body.usage as any;
        const mappedUsage: Record<string, number | string | undefined> = {};

        // Common keys from various SDKs/providers
        if (typeof ud.prompt_tokens === "number")
          mappedUsage.input = ud.prompt_tokens;
        if (typeof ud.completion_tokens === "number")
          mappedUsage.output = ud.completion_tokens;
        if (typeof ud.total_tokens === "number")
          mappedUsage.total = ud.total_tokens;

         if (typeof ud.input_cost === "number")
          mappedUsage.inputCost = ud.input_cost;
        if (typeof ud.output_cost === "number")
          mappedUsage.outputCost = ud.output_cost;
        if (typeof ud.total_cost === "number")
          mappedUsage.totalCost = ud.total_cost;

        if (typeof ud.unit === "string")
          mappedUsage.unit = ud.unit;

        // Fallback names that may already be used in some processors
        if (typeof ud.input === "number" && mappedUsage.input === undefined)
          mappedUsage.input = ud.input;
        if (typeof ud.output === "number" && mappedUsage.output === undefined)
          mappedUsage.output = ud.output;
        if (typeof ud.total === "number" && mappedUsage.total === undefined)
          mappedUsage.total = ud.total;

        if (typeof ud.inputCost === "number" && mappedUsage.inputCost === undefined)
          mappedUsage.inputCost = ud.inputCost;
        if (typeof ud.outputCost === "number" && mappedUsage.outputCost === undefined)
          mappedUsage.outputCost = ud.outputCost;
        if (typeof ud.totalCost === "number" && mappedUsage.totalCost === undefined)
          mappedUsage.totalCost = ud.totalCost;

        if (typeof ud.unit === "string")
          mappedUsage.unit = ud.unit;

        // If mapping produced any numeric values, attach as `usage` so the rest
        // of the function (which checks for `usage` presence) will use them.
        if (Object.keys(mappedUsage).length > 0) {
          // preserve any unit if available on usageDetails
          if (typeof ud.unit === "string") mappedUsage.unit = ud.unit;
          // Assign back to event body (mutating the parsed event is acceptable
          // here to keep downstream logic unchanged).
          (this.event.body as any).usage = mappedUsage as any;
        }
      } catch (e) {
        // If anything goes wrong, fall back to original behavior and don't block
        // ingestion.
        console.warn("Failed to normalize usageDetails to usage", e);
      }
    }
